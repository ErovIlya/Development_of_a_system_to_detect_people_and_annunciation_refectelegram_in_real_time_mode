...
async def download_video_for_task_1(message: types.Message, state: FSMContext) -> None:
    """
    ! Telegram handler [/video_task1]: Запуск задачи 1 (детектирование людей в видео файле)
...
async def start_task_2_for_found_people_for_bot(message: types.Message, state: FSMContext) -> None:
    """
    ! Telegram handler [/task2]: Запуск задачи 2 (детектирование людей и поиск пересечения с отрезком в видео файле)
    """
    if await check_task(state):
        await bot.send_message(
            message.chat.id,
...
async def download_video_for_task_2(message: types.Message, state: FSMContext) -> None:
    """
    ! Telegram handler [/video_task2]: Запуск задачи 2 (детектирование людей и поиск пересечения с отрезком в видео
