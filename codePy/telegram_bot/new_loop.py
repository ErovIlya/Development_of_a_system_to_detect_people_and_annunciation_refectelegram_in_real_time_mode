...
from codePy.handlers.simple_answer import send_message_error_in_stop, send_message_stop_complete
from supervision import LineZone
from codePy.classes import User, StateForTask2, StateForTask1
import threading
...
async def create_new_loop_for_task(path: str, chat_id: int, state: int, line: LineZone = None):
    ...
    if state in [StateForTask1.stream(), StateForTask1.search()]:
        ...
        new_thread = threading.Thread(target=start_execution_task2, args=(path, user, state, line))
        new_thread.start()
        list_chat_id_and_thread_id[chat_id] = new_thread.ident
    ...
    if chat_id in list_chat_id_and_thread_id:
        ...
        for thread in threading.enumerate():
            if thread.ident == thread_id:
                await send_message_stop_complete(chat_id, 'Остановка завершена')
